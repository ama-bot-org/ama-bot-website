...
const StandardLib: React.FC = () => {
  ...
  const initQATable = async (noSearch?: boolean) => {
    if (currentUser?.bot_id) {
      setLoading(true)
      try {
        const res = await standardLibAPI.getStandardTableInfo({
  ...
  const handleDeleteRow = async (record: QAFormInfo) => {
    if (currentUser?.bot_id) {
      setLoading(true)
      try {
        const res = await standardLibAPI.deleteStandardInfo({
          bot_id: currentUser.bot_id,
          id: record.id!,
        })
        if (res.ActionType === ActionType.OK) {
    ...
    <div className={containerClassName}>
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        <div className={topClassName}>
          <h3 className={headerTitleClassName}>优先使用自定义一对一标准回答</h3>
          <div className={searchWrapClassName}>
            <Form form={form} layout="inline">
              <Form.Item name="search">
                <Input.Search placeholder="搜主题/内容" className={searchClassName} onSearch={handleSearch} />
              </Form.Item>
            </Form>
            <ConfigProvider
              theme={{
